...
Insert into alumno
values ('18131209','José Adame','Calle 21A Fracc. Los Alamos','M','8712460707')

Insert into alumno 
values ('18131252','Jesús López','Av. Mina Col. Centro','M','8712792196')
 
Insert into alumno 
values ('18131282','Jose Rocha','Dom. Conocido Ejido la Paz','M','8718632123')
 
Insert into alumno 
values ('17131209','Axel Neave','Av. Falcon Col. Centro','M','8723658985')

Insert into alumno 
values ('17131259','Jannette Trujillo','Av. Solidaridad Col. Centauro','F','8712752036')
...
--Insertar información en la table profesor
...
--C15: Clave de las materias con la calificacion más baja
...
--C20: Cuantos alumnos han cursado la materia de Simulación
select count(no_control) from cursa 
WHERE clave_materia in(SELECT clave_materia FROM materia 
					   WHERE clave_materia = '0002')

--C21: Agregar ciudad a la tabla alumnos restringir a: Torreón, Gomez, Lerdo
alter table Alumno
...
--C24: Mostrar cuantos alumnos vivan en Torreón
select COUNT(*) from alumno 
where ciudad like 'Torreon'

--C25: Cuantos alumnos no viven en Torreón
select COUNT(*) from alumno 
...
--C35: Insertar inforamción de los departamentos a los profesores
update profesor
...
--C49: Cuantos Creditos lleva: 'José Rocha'
select SUM(no_creditos) from materia 
where clave_materia in(select clave_materia from cursa
					   where no_control in
...
--C57: Inforamción de los maestros de Sistemas ordenada por fecha de nacimiento en forma descendente
...
--C60: Cuantas materias hay en la BD, que imparte los maestros de Mecatrónica
